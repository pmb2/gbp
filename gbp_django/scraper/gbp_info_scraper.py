...
#!/usr/bin/env python3
"""
GBP Info Scraper: Retrieves a Google Business Profile (GBP) page using a direct HTTP request with Requests and BeautifulSoup.
If essential dynamic content is missing or the request fails (e.g. due to JavaScript‐rendered elements), falls back to using Selenium.
Collects business details, hours, menus, photos, reviews, posts, promos, locations, and SEO data.
NOTE: Update CSS selectors as needed based on your GBP page’s structure.
"""
...
def fetch_using_requests(url):
    ...
    try:
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code == 200:
            print("[SCRAPER] HTTP request succeeded.")
            return response.text
    except Exception as e:
        print(f"[SCRAPER] Requests error: {e}")
    return None
...
def init_selenium_driver():
    ...
    driver_path = "/path/to/chromedriver"
    driver = webdriver.Chrome(executable_path=driver_path, options=chrome_options)
    print("[SCRAPER] Selenium driver initialized.")
    return driver

def fetch_using_selenium(url):
    driver = init_selenium_driver()
    print(f"[SCRAPER] Selenium retrieving URL: {url}")
    driver.get(url)
    ...
    page_source = driver.page_source
    driver.quit()
    print("[SCRAPER] Selenium retrieval complete.")
    return page_source
...
def parse_gbp_page(html):
    ...
    try:
        data['name'] = soup.find("h1", class_="section-hero-header-title").get_text(strip=True)
        print(f"[SCRAPER] Extracted business name: {data['name']}")
    except Exception:
        data['name'] = None
        print("[SCRAPER] Failed to extract business name.")
    ...
    try:
        ...
        if hours_table:
            for row in hours_table.find_all("tr"):
                ...
                timing = row.find("td", class_="section-open-hours-time").get_text(strip=True)
                hours.append({"day": day, "time": timing})
            print(f"[SCRAPER] Extracted {len(hours)} hours entries.")
    except Exception:
        hours = None
        print("[SCRAPER] Failed to extract hours.")
    data['hours'] = hours
    ...
    try:
        ...
        reviews = None
        print("[SCRAPER] Failed to extract reviews.")
    data['reviews'] = reviews
    ...
    print("[SCRAPER] Parsing complete.")
    return data
...
def scrape_gbp_profile(business_url):
    ...
    if html:
        soup = BeautifulSoup(html, "html.parser")
        if soup.find("h1", class_="section-hero-header-title") is None:
            print("[SCRAPER] Essential dynamic content missing. Falling back to Selenium.")
            html = fetch_using_selenium(business_url)
    else:
        print("[SCRAPER] HTTP request failed. Using Selenium fallback.")
        html = fetch_using_selenium(business_url)
    ...
    data = parse_gbp_page(html)
    print("[SCRAPER] Scraping complete. Returning data.")
    return data
